...
func New() *gin.Engine {
	...
	createTenantCommand := commands.NewCreateTenantOnboardingCommand(tenantRepository, organizationRepository, priceRepository, billsRepository, iamOrganizationRepository, midtransService, publisherService)
	...
	createPaymentUrlCommand := commands.NewCreatePaymentURLCommand(billsRepository, midtransService)
	...
	billsControlerr := controller.NewBillController(*expireBillCommand, *payBillCommand, *createBillCommand, *checkPaymentCommand, *createPaymentUrlCommand, iamOrganizationQuery, billQueries)
